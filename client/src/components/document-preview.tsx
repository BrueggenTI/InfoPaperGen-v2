...
export default function DocumentPreview({ formData, sessionId }: DocumentPreviewProps) {
  ...
  const { claimsToShow, nutriScoreData } = useMemo(() => {
    ...
    if (formData.declarations?.manualClaims) {
      formData.declarations.manualClaims.filter(c => c.isActive).forEach(c => claims.push({ label: c.text, claim: "✓" }));
    }
      ...
      <div id="document-preview-content" className="pdf-preview-container bg-white rounded-xl shadow-2xl border border-slate-200">
        ...
        {formData.productImage && (
          <div className="product-image" style={{ textAlign: 'center', marginBottom: '12px' }}>
            <img src={formData.productImage} alt="Product" style={{ maxWidth: '300px', maxHeight: '128px', objectFit: 'contain', display: 'inline-block' }} />
          </div>
        )}
        ...
        {formData.nutrition && (
          <>
            ...
            <div className="grid-two-cols">
              {nutriScoreData && (
                <div className="section">
                  <div className="section-header"><h3 className="section-title">Nutri-Score Rating</h3></div>
              ...
              {claimsToShow.length > 0 && (
                <div className="section">
                  <div className="section-header"><h3 className="section-title">Possible declarations</h3></div>
                  <div className="section-content">
                    {claimsToShow.map(item => <div key={item.label} className="claim-item"><span>{item.label}</span><span className="claim-value" dangerouslySetInnerHTML={{__html: item.claim}}></span></div>)}
                  </div>
                </div>
              )}
            </div>
        ...
        <div className="section">
          ...
          <div className="section-content"><p>{formData.storageConditions || "..."}</p></div>
        </div>
        <div className="section">
          <div className="section-header"><h3 className="section-title">Allergy advice</h3></div>
          <div className="section-content"><p>{formData.allergyAdvice || "..."}</p></div>
        </div>
        <div className="section">
          <div className="section-header"><h3 className="section-title">Preparation instructions</h3></div>
          <div className="section-content"><p>{formData.preparation || "..."}</p></div>
        </div>
