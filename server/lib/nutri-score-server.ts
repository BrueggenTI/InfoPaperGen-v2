// Server-side Nutri-Score calculation utility (without image imports)
...
function calculateNutriGrade(finalScore: number): 'A' | 'B' | 'C' | 'D' | 'E' {
  ...
  if (finalScore >= 1 && finalScore <= 2) return 'B';
  if (finalScore >= 3 && finalScore <= 10) return 'C';
  if (finalScore >= 11 && finalScore <= 18) return 'D';
  return 'E'; // finalScore >= 19
}
...
/**
 * Get color class for Nutri-Score grade (server-side CSS color)
 */
export function getNutriScoreColor(grade: string): string {
  switch (grade) {
    case 'A': return 'background: #16a34a; color: white;';
    case 'B': return 'background: #84cc16; color: white;';
    case 'C': return 'background: #eab308; color: black;';
    case 'D': return 'background: #f97316; color: white;';
    case 'E': return 'background: #dc2626; color: white;';
