...
# [FIX] Installiert Chrome und seine Abhängigkeiten direkt in der finalen Stufe.
# Dieser Ansatz ist robuster als das manuelle Kopieren von Abhängigkeiten, da er
# sicherstellt, dass alle für die Laufzeit erforderlichen Bibliotheken vorhanden sind.
RUN apt-get update && apt-get install -y \
    wget gnupg ca-certificates fonts-liberation \
    libasound2 libatk-bridge2.0-0 libatk1.0-0 libcairo2 libcups2 \
    libdbus-1-3 libexpat1 libfontconfig1 libgbm1 libgcc1 libglib2.0-0 \
